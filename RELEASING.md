...
`github:mat3ra/ide#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/ide/releases/download/wip-<short-sha>/ide.tgz
```
...
```bash
npm run mat3ra:install -- ide wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/ide@<release-url>` with
