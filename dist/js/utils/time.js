...
function timestampToDate(timestamp = false, millisec = false) {
  return timestamp ? (0, _moment.default)(timestamp * (millisec ? 1 : 1000)).format("MMM D, YYYY, HH:mm A") : "";
}

function daysAgoToDate(days) {
  return (0, _moment.default)().utc().startOf("day").subtract(days, "days").toDate();
}